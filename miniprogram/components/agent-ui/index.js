...
Component({
  ...
  attached: async function () {
    ...
    if (this.data.demoMode && chatMode === "bot") {
      const record = {
        content: "你好，我是科研合作助手（演示）。请描述你的合作需求，我会给出 3~5 位教授推荐卡片。",
        record_id: "record_id" + String(+new Date() + 10),
        role: "assistant",
        hiddenBtnGround: true,
      };
      const demoBot = {
        botId: this.data.agentConfig?.botId || "demo-bot",
        name: "",
        welcomeMessage: "",
  ...
  methods: {
    ...
    sendMessage: async function (message) {
      ...
      // 新增一轮对话记录时 自动往下滚底
      this.autoToBottom();
      // demoMode: 本地演示，直接注入模拟的教授推荐卡片，跳过云端
      if (this.data.demoMode) {
        const newValue = [...this.data.chatRecords];
        const lastValueIndex = newValue.length - 1;
        const lastValue = newValue[lastValueIndex];
        lastValue.content = "这是根据你的需求生成的初步推荐，供演示使用。";
        lastValue.toolCallList = [
          {
            id: "demo-prof-rec",
            name: "professor_recommend",
            rawParams: { intent: inputValue },
            callParams: "",
            content: "",
            rawResult: {
              content: [
                    ...
                    candidates: [
                      {
                        ...
                        school: "计算机科学与技术学院",
                        areas: ["自然语言处理", "推荐系统"],
                        homepage: "https://www.example.com/zhangsan",
                        email: "zhangsan@zju.edu.cn",
                        highlights: [
                          "近三年在NLP方向发表顶会论文6篇",
                          "承担国家重点研发计划子课题，方向：行业知识图谱",
                        ],
                        score: 0.91,
                      },
                      {
                        ...
                        school: "控制科学与工程学院",
                        areas: ["计算机视觉", "多模态理解"],
                        homepage: "https://www.example.com/lisi",
                        email: "lisi@zju.edu.cn",
                        highlights: [
                          "主持省重点实验室项目，方向：工业视觉检测",
                          "与龙头企业合作落地多个多模态识别项目",
                        ],
                        score: 0.88,
                      },
                      {
                        ...
                        school: "软件学院",
                        areas: ["数据挖掘", "可解释AI"],
                        homepage: "https://www.example.com/wangwu",
                        email: "wangwu@zju.edu.cn",
                        highlights: [
                          "提出基于图学习的可解释推荐方法，获得最佳论文",
                          "与政府合作建设科研项目管理平台",
                        ],
                        score: 0.85,
                      },
